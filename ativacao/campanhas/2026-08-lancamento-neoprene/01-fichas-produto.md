# Etapa 1 — Fichas de produto extraídas das artes

**Fonte:** 21 artes entregues em `front.zip` (23 arquivos, 2 duplicatas exatas), convertidas
de `.webp` para `.jpg` e arquivadas em `imagens-design/`.
**Data da leitura:** 09/08/2026.
**Regra aplicada:** todo claim abaixo é transcrição do que a arte afirma. O que não
aparece em nenhuma arte está marcado como **não consta na arte**, sem inferência.

> Nota de formato: as artes chegaram em WebP. Convertidas para JPG porque WebP não
> renderiza em boa parte dos clientes de email — Outlook desktop e clientes antigos
> mostram imagem quebrada.

## Os dois grupos de arte

O acervo se divide em dois, com destinos diferentes:

**Assets do email — 9 fotografias.** Packshots e lifestyle, sem texto aplicado. São as
únicas imagens que entram na peça.

| Arquivo | Produto | O que mostra |
|---|---|---|
| `00-linha-neoprene/linha-hero-surfista-01.jpg` | Linha | **Hero.** Surfista saindo da água de frente, luz de fim de tarde, camiseta e bermuda, prancha sob o braço. Horizontal, 1200 × 755 |
| `00-linha-neoprene/linha-lifestyle-surfista-02.jpg` | Linha | Mesma sessão, enquadramento vertical de corpo inteiro. 1200 × 1393 |
| `00-linha-neoprene/linha-lifestyle-praia-01.jpg` | Linha | Surfista de costas na praia usando os três produtos, **sapatilhas incluídas**, com prancha e leash |
| `01-camiseta-neoprene-cabo-frio/camiseta-packshot-frente.jpg` | Camiseta | Frente em modelo, gola alta, etiqueta OH circular no peito |
| `01-camiseta-neoprene-cabo-frio/camiseta-packshot-costas.jpg` | Camiseta | Costas em modelo, recorte raglã e costura central |
| `02-bermuda-neoprene-joaquina/bermuda-packshot-lateral.jpg` | Bermuda | Lateral em modelo, close do caimento e da etiqueta emborrachada |
| `02-bermuda-neoprene-joaquina/bermuda-lifestyle-01.jpg` | Bermuda | Corpo inteiro em modelo, ao ar livre |
| `03-sapatilha-esportiva-neoprene/sapatilha-packshot-lateral.jpg` | Sapatilha | Perfil da peça em fundo claro |
| `03-sapatilha-esportiva-neoprene/sapatilha-packshot-solado.jpg` | Sapatilha | Solado voltado para a câmera, textura antiderrapante visível |

**Referências de conteúdo — 9 infográficos**, em `<produto>/referencia/`. Trazem título,
ícones e texto sobre a foto. **Não entram no email.** Servem para extrair a informação
técnica que alimenta a copy e para o Design ver como a marca já comunicou cada feature.
O tratamento visual das features na peça é criação do Design, não recorte destas artes.

Consequência a registrar: a peça tem **9 fotografias para três produtos**. Se o layout
pedir mais ângulos, é pedido de foto nova, não de recorte de infográfico.

### Nota sobre as duas fotos novas (10/08/2026)

`linha-hero-surfista-01.jpg` e `linha-lifestyle-surfista-02.jpg` chegaram com nome de
arquivo do Gemini e **marca d'água de IA** — duas estrelas de quatro pontas na horizontal
e uma na vertical. Removidas por corte: 312 px da direita na horizontal, 310 px da base
na vertical. Os cortes foram conferidos e as versões arquivadas estão limpas.

**Restrição que fica:** nas duas, o texto de marca aplicado nas peças está embaralhado —
o selo circular no peito tem letras ilegíveis sob o "OH", e a etiqueta na coxa da bermuda
é um borrão. Na largura de exibição do email o peito fica com cerca de 30 px, e nesse
tamanho lê como selo, não como texto errado. **Não ampliar nem recortar o peito ou a
etiqueta da coxa** — é aí que o defeito aparece.

---

## Assinatura da linha

Os três produtos compartilham **neoprene de 1,5 mm**. Isso aparece em todos:

| Produto | Onde o 1,5 mm aparece | Arte |
|---|---|---|
| Camiseta Cabo Frio | Neoprene Span/Flex 1,5 mm no corpo | `camiseta-infografico-neoprene-spanflex.jpg` |
| Bermuda Joaquina | Neoprene nylon/nylon 1,5 mm no corpo; open cell/nylon 1,5 mm na barra | `bermuda-infografico-tecnologia.jpg` |
| Sapatilha | Neoprene nylon 1,5 mm no contorno do pé | `sapatilha-infografico-tecnologia.jpg` |

Segundo eixo comum: **construção em tecidos combinados**, com poliamida onde precisa de
elasticidade e leveza (gola, mangas, recortes e punho da sapatilha) e neoprene onde
precisa de térmica e fixação. Os três produtos são pretos.

---

## Camiseta de neoprene Cabo Frio

**Artes lidas:** `camiseta-packshot-frente.jpg`, `camiseta-packshot-costas.jpg`,
`camiseta-infografico-neoprene-spanflex.jpg`, `camiseta-infografico-mangas-recortes.jpg`,
`camiseta-infografico-gola.jpg`, `camiseta-infografico-punhos.jpg`,
`camiseta-infografico-etiqueta.jpg`

**Construção:** manga longa, gola alta, raglã. Corpo em neoprene Span/Flex 1,5 mm;
mangas, recortes laterais e gola em poliamida premium com elastano; punhos em neoprene.

| Campo | Conteúdo |
|---|---|
| Preço | **não consta na arte** |
| Material do corpo | Neoprene Span/Flex 1,5 mm |
| Fornecedor do neoprene | A arte cita "Inneo Brasil" na abertura e "Inrneo Brasil" no bullet — **grafias divergentes na mesma peça** |
| Material de mangas, recortes e gola | Poliamida premium com elastano, UV 50 |
| Material dos punhos | Neoprene |
| Proteção solar | UV 50+ (mangas, recortes laterais, gola e punhos) |
| Tamanhos | **não consta na arte** |
| Cores | Preto (única cor mostrada) |
| Frete e prazo | **não consta na arte** |
| Provas, selos, avaliações | **não consta na arte** |

**Claims transcritos:**

- "Neoprene Span/Flex 1,5mm — tecnologia, conforto e performance" — `spanflex`
- "Mantém a temperatura do corpo, protegendo contra o vento e o frio da água" — `spanflex`
- "Elasticidade 360° — alta flexibilidade que proporciona liberdade total de movimentos
  dentro e fora da água" — `spanflex`
- "Resistente ao uso intenso, ao cloro e à água salgada, garantindo maior vida útil" — `spanflex`
- "Tecido de poliamida premium com UV 50" — `mangas-recortes`
- "Proteção UV 50+ — bloqueia os raios solares, protegendo sua pele durante longas
  exposições ao sol" — `mangas-recortes`, `gola`
- "Alta resistência ao cloro, sal e ao uso intenso, mantendo a cor e a performance por
  muito mais tempo" — `mangas-recortes`
- "Gola desenvolvida no mesmo tecido técnico de poliamida utilizado nas mangas e recortes
  laterais" — `gola`
- "Elasticidade multidirecional que proporciona liberdade de movimento e ajuste perfeito
  ao pescoço" — `gola`
- "O punho em neoprene mantém a manga firme no braço, evitando que se mova ou escorregue
  durante os movimentos" — `punhos`
- "Permite foco total no esporte, sem distrações ou ajustes constantes na manga" — `punhos`
- "Etiqueta emborrachada — não desbota: proteção UV que mantém a cor e o relevo por muito
  mais tempo" — `etiqueta`

**Assets:** `camiseta-packshot-frente.jpg`, `camiseta-packshot-costas.jpg`.
**Referências lidas:** os cinco infográficos em `referencia/`.

**Lacunas:** grade de tamanhos, tabela de medidas, prova social. (Preço e prazo de
entrega ficam fora da peça por decisão editorial, não por falta de dado.)

---

## Bermuda de neoprene Joaquina

**Artes lidas:** `bermuda-packshot-lateral.jpg`, `bermuda-lifestyle-01.jpg`,
`bermuda-infografico-tecnologia.jpg`, `bermuda-infografico-cordao-cos.jpg`,
`bermuda-infografico-etiqueta.jpg`

**Construção:** bermuda colada ao corpo, comprimento acima do joelho, com cordão interno
no cós e costura flat.

| Campo | Conteúdo |
|---|---|
| Preço | **não consta na arte** |
| Material do corpo | Neoprene nylon/nylon 1,5 mm |
| Material da barra | Neoprene open cell/nylon 1,5 mm |
| Costura | Costura técnica flat |
| Ajuste | Cordão interno no cós, totalmente interno |
| Etiqueta | PVC emborrachado |
| Proteção solar | **não consta na arte** (a arte da etiqueta cita resistência ao sol, não proteção UV para o corpo) |
| Tamanhos | **não consta na arte** |
| Cores | Preto (única cor mostrada) |
| Frete e prazo | **não consta na arte** |
| Provas, selos, avaliações | **não consta na arte** |

**Claims transcritos:**

- "Neoprene de alta performance com revestimento nylon/nylon, que oferece mais
  elasticidade, conforto e liberdade de movimento" — `tecnologia`
- Barra em open cell: "absorve menos água", "secagem mais rápida", "mais leve e flexível" — `tecnologia`
- "Costura profissional que proporciona alta resistência, flexibilidade e conforto.
  Acabamento premium que evita atrito na pele" — `tecnologia`
- "Compressão e ajuste", "conforto térmico", "resistência à água salgada" — `tecnologia`
- "O cordão interno permite ajustar a bermuda perfeitamente ao corpo, garantindo firmeza
  sem apertar" — `cordao-cos`
- "Mantém a bermuda no lugar durante os movimentos, dentro e fora d'água" — `cordao-cos`
- "O cordão fica totalmente interno, preservando o visual clean da peça" — `cordao-cos`
- "Produzida em PVC emborrachado de alta durabilidade, resistente à água salgada, sol e
  uso intenso" — `etiqueta`
- "Não descasca nem racha" — `etiqueta`

**Assets:** `bermuda-packshot-lateral.jpg`, `bermuda-lifestyle-01.jpg`.
**Referências lidas:** os três infográficos em `referencia/`. O de tecnologia é o mais
rico do acervo — separa corpo e barra em dois materiais distintos, com macro de cada
tecido. É a melhor fonte de conteúdo da campanha, e vale como referência de como a marca
já explicou essa dupla construção.

**Lacunas:** grade de tamanhos, tabela de medidas, prova social, proteção UV.

---

## Sapatilha esportiva de neoprene

**Artes lidas:** `sapatilha-packshot-lateral.jpg`, `sapatilha-packshot-solado.jpg`,
`sapatilha-infografico-tecnologia.jpg`

**Construção:** três tecidos técnicos — solado, punho e contorno do pé.

| Campo | Conteúdo |
|---|---|
| Preço | **não consta na arte** |
| Solado | Neoprene Shak, antiderrapante |
| Punho | Poliamida opaca, punho duplo |
| Contorno do pé | Neoprene revestido com nylon em ambas as faces, 1,5 mm |
| Numeração | **não consta na arte** |
| Cores | Preto (única cor mostrada) |
| Frete e prazo | **não consta na arte** |
| Provas, selos, avaliações | **não consta na arte** |

**Claims transcritos:**

- "Solado antiderrapante com alta aderência e resistência para máxima segurança" — `tecnologia`
- "Antiderrapante", "alta durabilidade", "resistência à abrasão", "flexível e resistente" — `tecnologia`
- "Punho duplo desenvolvido em poliamida com alta elasticidade e toque macio" — `tecnologia`
- "Leve e confortável", "ajuste perfeito à perna", "secagem rápida", "maior liberdade de
  movimento" — `tecnologia`
- "Corpo da sapatilha em neoprene revestido com nylon em ambas as faces, proporcionando
  conforto térmico e flexibilidade" — `tecnologia`
- "Isolamento térmico", "alta flexibilidade", "secagem rápida", "resistente à água e ao
  vento" — `tecnologia`
- "A combinação de três tecidos técnicos proporciona o equilíbrio perfeito entre
  proteção, conforto, aderência e liberdade de movimento" — `tecnologia`

**Assets:** `sapatilha-packshot-lateral.jpg`, `sapatilha-packshot-solado.jpg`.
**Referências lidas:** `referencia/sapatilha-infografico-tecnologia.jpg`, que numera três
pontos sobre a foto e liga cada um a um macro de tecido.

**Falha registrada na arte:** "PROTEÇÃO DENTRO E **FORAA** D'ÁGUA". Como o infográfico é
referência e não asset, o erro não chega ao email — mas vale corrigir na arte, que
provavelmente circula em outros canais.

**Lacunas:** numeração disponível, prova social.

---

## Linha — artes de contexto

Três fotos mostram a linha vestida, e cada uma tem um papel diferente:

- **`linha-hero-surfista-01.jpg`** — horizontal, surfista de frente saindo da água na
  luz de fim de tarde. É o hero: o formato cabe na dobra sem comer uma tela inteira de
  altura, e a espuma à esquerda deixa área livre para a headline. Mostra camiseta e
  bermuda; o enquadramento termina na canela, então a ausência da sapatilha não aparece.
- **`linha-lifestyle-praia-01.jpg`** — surfista de costas usando **os três produtos ao
  mesmo tempo**, sapatilhas nos pés. É a única que prova que a linha tem três peças, e
  por isso vale mais no fechamento, onde a copy reamarra as três.
- **`linha-lifestyle-surfista-02.jpg`** — vertical de corpo inteiro da mesma sessão do
  hero. Sobra para o Design usar onde a composição pedir.

## Marca

`04-marca/logo-use-zero-hora-branco.png` — logo oficial em versão negativa: símbolo em
"Z" mais o lettering "USE ZERO HORA" em três linhas, branco sobre fundo transparente.
**Só funciona sobre fundo escuro.** A versão positiva ainda não existe no repositório.

---

## Artes fora da linha neoprene

Cinco das 21 artes não são desta linha. Ficaram arquivadas em
`imagens-design/99-fora-da-linha-neoprene/` e **não entram no email**:

| Arquivo | O que é | Por que ficou fora |
|---|---|---|
| `lycra-azul-manga-longa.jpg` | Camiseta de lycra azul, manga longa, logo "ZERO HORA" escrito | Outro produto e outra linha |
| `lycra-azul-manga-curta.jpg` | Camiseta de lycra azul, manga curta | Outro produto e outra linha |
| `lycra-verde-limao-01.jpg` | Lycra preta e verde-limão com a marca em "Z" | Outro produto e outra linha |
| `lycra-verde-02.jpg` | Lycra verde e preta com a marca em "Z" | Mesmo caso |
| `lycra-preta-01.jpg` | Lycra preta com a marca em "Z" | Mesmo caso |

**Correção de 10/08/2026.** A versão anterior desta seção afirmava que a marca em "Z"
dessas três peças "não é o logo da marca". Está errado: o logo oficial recebido em
10/08 é exatamente esse "Z" ao lado do lettering "USE ZERO HORA". As peças estão com a
marca correta. O motivo de ficarem fora continua sendo outro — são lycra, não neoprene.

---

## Fora da peça por decisão editorial

Não são lacunas: são omissões deliberadas, confirmadas em 09/08/2026.

- **Preço.** O email vende a linha pelo benefício de uso e pela tecnologia; o preço
  aparece na página do produto, depois do clique.
- **Prazo de entrega.** Fora da peça.

Registrado aqui para que nenhuma etapa seguinte trate a ausência como esquecimento e
tente preencher.

## Lacunas abertas

1. **Grade de tamanhos e numeração.** Nenhuma arte traz. "Qual tamanho eu compro" é
   objeção clássica em roupa de água colada ao corpo, e a peça não tem resposta.
2. **Prova social.** Nenhuma avaliação, selo ou depoimento em nenhuma arte.
3. **Proteção UV da bermuda.** A camiseta declara UV 50+; a bermuda não declara nada.
   Escrever "linha com proteção UV" seria estender à bermuda um atributo que só a
   camiseta comprova.
4. **Ângulos de foto.** Sete fotografias para três produtos, e só a bermuda tem foto de
   corpo inteiro. Se o layout pedir mais, é foto nova.

## Falhas nas artes de referência

Não chegam ao email, porque as artes de referência não entram na peça. Valem correção
porque provavelmente circulam em outros canais.

- **Grafia do fornecedor do neoprene**: a arte da camiseta escreve "Inneo Brasil" na
  abertura e "Inrneo Brasil" no bullet.
- **Typo na arte da sapatilha**: "PROTEÇÃO DENTRO E FORAA D'ÁGUA".
